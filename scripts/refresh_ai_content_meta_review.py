...
- Run at least {MIN_QUERY_ITERATIONS} distinct search iterations across multiple query phrasings and source types.
- Search both date windows: very recent sources since {recent_publication_date.isoformat()} and broader missed {today.year} sources since {earliest_publication_date.isoformat()}.
- Search academic and preprint lanes: arXiv, PubMed, SSRN, ACM, ACL Anthology, IEEE, Springer, Elsevier, Nature, ScienceDirect, and journal/conference pages.
- Search industry and platform lanes: AI detector companies, SEO/content platforms, social platforms, app stores, review platforms, music/image/video platforms, publishing platforms, and analytics firms.
- Search institutional and policy lanes: universities, research institutes, government reports, regulator reports, think tanks, observatories, and standards bodies.
- Search media and analyst lanes only as discovery paths; prefer the primary source they cite.
- Search for recent estimates across web pages, social posts, reviews, academic writing, news, images, video, music, code, product listings, app reviews, and platform-specific corpora.
- Use varied query forms, including exact phrases like "AI-generated content share", "LLM-generated", "AI-written", "synthetic content", "generated by AI", "AI assisted", "content prevalence", "percentage of", "share of", "study", "dataset", "estimate", and {today.year}.
...
- For each promising source, open or inspect enough context to verify the denominator, year, method, and publication date before including it.
- Stop only after you have searched all major lanes or found enough evidence that no qualifying recent sources exist.
...
- In remaining_gaps, state which lanes looked thin or had no qualifying recent estimates.
